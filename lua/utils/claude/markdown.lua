...
local function parse_inline(line)
	...
	while i <= n do
		if line:sub(i, i + 1) == "**" then
			...
			-- Require a non-space char adjacent to both delimiters — otherwise this
			-- is arithmetic-like prose ("3 ** 2 ** 5"), not emphasis, and treating it
			-- as bold silently eats the markers around it.
			if close and not (line:sub(i + 2, i + 2):match("%s") or line:sub(close - 1, close - 1):match("%s")) then
...
local TBL = {
	...
	TL = "┌",
	TM = "┬",
	TR = "┐", -- top    rule corners/tee
	ML = "├",
	MM = "┼",
	MR = "┤", -- header divider corners/tee
	BL = "└",
	BM = "┴",
	BR = "┘", -- bottom rule corners/tee
}
...
local function disp_take(s, w)
	...
	if out == "" then
		-- Always consume ≥ 1 whole CODEPOINT (not 1 byte) — a bare `s:sub(1, 1)`
		-- fallback split a multibyte char mid-sequence, and wrap_text's hard-break
		-- loop then kept re-splitting the mangled remainder byte-by-byte, shattering
		-- table box-frame alignment on any CJK cell narrower than one glyph.
		local b = s:byte(1)
		local len = (b < 0x80 and 1) or (b < 0xE0 and 2) or (b < 0xF0 and 3) or 4
		out = s:sub(1, len)
	end
...
local function render_table(rows)
	...
	if #raw_rows == 0 then
		-- A run consisting only of separator rows (no real data row) isn't a table
		-- at all — fall back to plain inline-parsed text instead of vanishing.
		local out_lines, out_hls = {}, {}
		for _, r in ipairs(rows) do
			local clean, ihls = parse_inline(r)
			out_lines[#out_lines + 1] = clean
			out_hls[#out_hls + 1] = ihls
		end
	...
	for c = 1, ncols do
		-- Floor at 2, not 1: a width-1 column can't fit a single width-2 (CJK/emoji)
		-- codepoint, so disp_take's main loop never takes anything and every cell
		-- falls to its single-codepoint fallback — floor of 2 gives it room.
		if widths[c] < 2 then
			widths[c] = 2
		end
	end
	...
	for ri, cells in ipairs(raw_rows) do
		...
		for k = 1, height do -- one physical line per wrap row
			...
			segs[#segs + 1] = TBL.V
			hls[#hls + 1] = { blen, blen + #TBL.V, "ClaudeDim" }
			blen = blen + #TBL.V
			for c = 1, ncols do
				segs[#segs + 1] = " "
				blen = blen + 1
				-- wrap_text sized wrapped[c][k] to widths[c] BEFORE inline parsing, but
				-- parse_inline (span_style) can WIDEN it — a devicon insertion adds a
				-- glyph — so `clean` can exceed widths[c]. Clamp with a codepoint-safe
				-- disp_take (not a byte slice) so the row can't run past the frame, and
				-- clip any highlight span that fell past the clamp.
				local clean_full, ih = parse_inline(wrapped[c][k] or "")
				local clean = disp_take(clean_full, widths[c])
				for _, h in ipairs(ih) do
					local he = math.min(h[2], #clean)
					if he > h[1] then
						hls[#hls + 1] = { blen + h[1], blen + he, h[3] }
					end
				end
				if ri == 1 then
					hls[#hls + 1] = { blen, blen + #clean, "ClaudeBold" }
				end
				segs[#segs + 1] = clean
				blen = blen + #clean
		...
		if ri == 1 then
			push(table_rule(TBL.ML, TBL.MM, TBL.MR, widths))
		end -- header divider
	end
	...
	while idx <= #rest do
		...
		elseif #tok > 1 and tok:sub(-1) == "/" then
			...
			if ext then
				local ok, dev = pcall(require, "nvim-web-devicons")
				if ok then
					icon, hl = dev.get_icon(fname, ext, { default = false })
				end
			end
			if icon and hl then
...
local function is_heading(s)
	return s:match("^#+%s") ~= nil and #s:match("^(#+)") <= 6
end
...
local function is_quote(s)
	return s:match("^%s*>") ~= nil
end
local function is_list_item(s)
...
local function render_heading(line)
	...
	-- Require whitespace before the closing run (proper ATX-close syntax) — an
	-- unconditional strip ate heading text that legitimately ends in "#" (e.g. "# C#").
	...
	for _, h in ipairs(ihls) do
		hls[#hls + 1] = { #prefix + h[1], #prefix + h[2], h[3] }
	end
	return padded, hls
end
...
local function render_quote(line)
	...
	for _, h in ipairs(ihls) do
		hls[#hls + 1] = { #QUOTE_BAR + h[1], #QUOTE_BAR + h[2], h[3] }
	end
	return out, hls
end
...
local CODE_CAP = {
	...
	keyword = "ClaudeCodeKeyword",
	conditional = "ClaudeCodeKeyword",
	["repeat"] = "ClaudeCodeKeyword",
	label = "ClaudeCodeKeyword",
	exception = "ClaudeCodeKeyword",
	include = "ClaudeCodeKeyword",
	-- operators / punctuation
	operator = "ClaudeCodeOper",
	punctuation = "ClaudeCodePunc",
	-- literals
	string = "ClaudeCodeString",
	character = "ClaudeCodeString",
	number = "ClaudeCodeNumber",
	float = "ClaudeCodeNumber",
	boolean = "ClaudeCodeNumber",
	-- comments
	comment = "ClaudeCodeComment",
	-- callables
	["function"] = "ClaudeCodeFunc",
	method = "ClaudeCodeFunc",
	constructor = "ClaudeCodeFunc",
	-- types / modules / tags
	type = "ClaudeCodeType",
	module = "ClaudeCodeType",
	namespace = "ClaudeCodeType",
	tag = "ClaudeCodeType",
	-- constants / attributes
	constant = "ClaudeCodeConst",
	attribute = "ClaudeCodeConst",
	-- identifiers
	variable = "ClaudeCodeVar",
	property = "ClaudeCodeVar",
	field = "ClaudeCodeVar",
	parameter = "ClaudeCodeVar",
	-- markup / prose / diff (markdown-in-code, legacy @text.*, diff fences)
	markup = "ClaudeCodeVar",
	text = "ClaudeCodeVar",
	diff = "ClaudeCodeVar",
	title = "ClaudeCodeKeyword",
	uri = "ClaudeCodeString",
	math = "ClaudeCodeNumber",
	environment = "ClaudeCodeType",
	note = "ClaudeCodeComment",
	warning = "ClaudeCodeComment",
	danger = "ClaudeCodeComment",
	todo = "ClaudeCodeComment",
	error = "ClaudeCodeComment",
}
...
local function code_ts_hls(lang, body)
	...
	-- The fence-lang info string is fully untrusted (model prose, or quoted out of
	-- a file the agent read) and resolves+dlopens a runtime parser file by name.
	-- nvim's own vim.treesitter.language.add guard already rejects anything
	-- outside [%w_], but that guard is upstream and version-dependent — clamp
	-- here too as defense-in-depth rather than relying on it silently.
	if not plang:match("^[%w_]+$") then
		return nil
	end
	...
	if not got_q or not query then
		return nil
	end
...
local function body_is_tree(body)
	...
	for _, l in ipairs(body) do
		if l:find("├", 1, true) or l:find("└", 1, true) or l:find("│", 1, true) or l:find("──", 1, true) then
			n = n + 1
			if n >= 2 then
				return true
			end
...
local function build_md_lines(raw)
	...
	while idx <= #raw do
		...
		elseif is_heading(line) then
			push(render_heading(line))
			idx = idx + 1
		elseif is_hrule(line) then
			push(render_hrule())
			idx = idx + 1
		elseif is_quote(line) then
			push(render_quote(line))
			idx = idx + 1
		elseif is_list_item(line) then
			push(render_list_item(line))
			idx = idx + 1
		elseif is_tree_line(line) then
			push(render_tree_line(line))
			idx = idx + 1
		else
			push(parse_inline(line))
			idx = idx + 1
		end
	end
