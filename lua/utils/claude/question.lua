...
local function q_choice(q)
	return q.choice[q.qi] or 1
end
...
local function render_question_card()
	local q = state.qask
	if not (q and q.buf and vim.api.nvim_buf_is_valid(q.buf) and q.win and vim.api.nvim_win_is_valid(q.win)) then
		return
	end
	...
	local title = (#q.questions > 1) and (" ❓ Question " .. q.qi .. " of " .. #q.questions .. " ") or " ❓ Question "
...
local function toggle_question_choice()
	local q = state.qask
	if not (q and q.questions[q.qi].multiSelect) then
		return
	end
	local d = question_display_options(q.questions[q.qi])[q_choice(q)]
	if not (d and d.kind == "model") then
		return
	end
...
local function close_question_card(receipt, receipt_hl)
	...
	if q.resize_close then
		pcall(q.resize_close)
...
local function open_question_float(q)
	...
	vim.api.nvim_create_autocmd("WinClosed", {
		...
		once = true,
		callback = function()
			if state.qask and state.qask.win == win then
				cancel_question()
			end
		end,
	})
...
show_question_card = function(event)
	...
	if pet_emit then
		pet_emit("permission", { active = true, build = false })
	end
