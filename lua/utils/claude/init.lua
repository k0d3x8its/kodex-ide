...
local function attach_panel_float_resize(win, group_name, on_resize)
  ...
  vim.api.nvim_create_autocmd({ "VimResized", "WinResized" }, {
    ...
    callback = function()
      ...
      c.col, c.row, c.width = col, widgets.float_bottom_row(), w
...
-- space, which the chat-bar/float machinery here computes). Deferred to here because
-- all three are now defined. 15.5/15.7 may re-home these; only this call changes.
widgets.wire({
...
-- Re-export the question card's test hooks + the dispatcher entry point so the
-- `mod._question*` spec references (and the show_question_card dispatch call below)
-- resolve; they moved to claude/question.lua in the Goal 15.4 extraction.
local show_question_card               = question.show_question_card
