return {
	...
	config = function()
		...
		-- The alpha window, if one is currently displayed anywhere (nil otherwise).
		...
		local initial_ok, initial_err = pcall(populate_recent)
		if not initial_ok then
			vim.notify("alpha recent-files populate failed: " .. tostring(initial_err), vim.log.levels.WARN)
		end
		...
		vim.api.nvim_create_autocmd({ "VimResized", "WinResized", "WinClosed", "WinNew" }, {
			...
			callback = function()
				local has_alpha = find_alpha_win() ~= nil
				...
				vim.schedule(function()
					...
					if not ok then
						vim.notify("alpha recent-files reflow failed: " .. tostring(err), vim.log.levels.WARN)
					end
				end)
			end,
		})
