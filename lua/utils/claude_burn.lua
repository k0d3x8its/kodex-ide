...
local function read_state()
  ...
  if cache.data and cache.mtime == mtime then return cache.data end
  local fd = io.open(STATE, "r")
  if not fd then return cache.data end
  local raw = fd:read("*a")
  fd:close()
  local ok, decoded = pcall(vim.json.decode, raw)
  if not ok or type(decoded) ~= "table" then return cache.data end
  cache.mtime = mtime
  cache.data  = decoded
  return decoded
end
...
local function countdown(resets_at)
  if type(resets_at) ~= "number" then return "" end
...
local function severity(pct)
  if pct >= 85 then return "Crit" end
...
-- false → just "<glyph> <label> NN%".
local function add_meter(out, glyph, label, pct, resets_at, show_bar)
  ...
  local sev = "ClaudeBurn" .. severity(pct)
  ...
  out[#out + 1] = { tostring(math.floor(pct + 0.5)) .. "%", sev }
  if type(resets_at) == "number" then
    out[#out + 1] = { " ↻" .. countdown(resets_at), "ClaudeBurnReset" }
  end
end
...
local function build(rl, cw, show_bar, show_reset)
  local out = {}
  local function gap() if #out > 0 then out[#out + 1] = { "  ", "ClaudeBurnLabel" } end end
  ...
  if type(rl.five_hour) == "table" then
    add_meter(out, GLYPH.five, "5h", rl.five_hour.used_percentage,
      show_reset and rl.five_hour.resets_at or nil, show_bar)
  end
  if type(rl.seven_day) == "table" then
    gap()
    add_meter(out, GLYPH.seven, "7d", rl.seven_day.used_percentage,
      show_reset and rl.seven_day.resets_at or nil, show_bar)
  end
...
local function chunks_width(chunks)
  local w = 0
  for _, t in ipairs(chunks) do w = w + vim.fn.strdisplaywidth(t[1]) end
  return w
end
...
function M.chunks(maxwidth)
  ...
  for _, lvl in ipairs({ { true, true }, { true, false }, { false, false } }) do
    ...
    if chunks_width(out) <= maxwidth then return out end
  end
