...
local function active_index()
  for i, o in ipairs(OPTIONS) do
    if o.id == state.advisor_model then return i end
  end
  return nil
end
...
local function render()
  if not (modal.buf and vim.api.nvim_buf_is_valid(modal.buf)) then return 0 end
  ...
  local function add(text, hl, col0, col_end)
    lines[#lines + 1] = text
    if hl then hls[#hls + 1] = { #lines - 1, col0 or 0, col_end or -1, hl } end
  end

  add(" Advisor (experimental)", "ClaudeEffortTitle")
  add("")
  for _, l in ipairs(wrap(BLURB, textw)) do add(" " .. l, "ClaudeSlashDesc") end
  add("")
  ...
  add("")
  for _, l in ipairs(wrap(RECO, textw)) do add(" " .. l, "ClaudeSlashDesc") end
  add("")
...
local function move(delta)
  if not Advisor.active() then return end
...
function Advisor.open(on_confirm)
  if Advisor.active() then Advisor.close() end
  ...
  modal.win = vim.api.nvim_open_win(modal.buf, true, {
    ...
    row = bottom, col = col, width = width, height = 14,
  ...
  if modal.win and vim.api.nvim_win_is_valid(modal.win) then
    vim.api.nvim_win_set_config(modal.win, {
      relative = "editor", anchor = "SW",
      row = widgets.float_bottom_row(), col = col, width = width, height = n,
    })
  end
end
