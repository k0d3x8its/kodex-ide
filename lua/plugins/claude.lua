...
return {
	...
	init = function()
		...
		local function define_highlights()
			...
			-- Subagent drill-in view title tag — dark text on a green fill, pinned top-right
			...
			vim.api.nvim_set_hl(0, "ClaudeBurnStale", { fg = "#6272a4", italic = true }) -- window rolled over, file % unreliable
	...
	keys = {
		...
		},
		{
			"<leader>cm",
			function()
				require("utils.claude").pick_model()
			end,
		...
		},
		{
			"<leader>cp",
			function()
				require("utils.claude").toggle_plan()
			end,
		...
		},
		{
			...
			"<leader>cb",
			function()
				require("utils.claude").toggle_host_ctx()
			end,
		...
		},
		{
			...
			"<leader>cq",
			function()
				require("utils.claude").ask_selection()
			end,
			mode = "v",
