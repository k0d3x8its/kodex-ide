...
local function open_diff(path)
  ...
  vim.keymap.set("n", "<leader>oa", M.accept_all,
    { buffer = scratch, desc = "OpenCode diff: accept all" })
  vim.keymap.set("n", "<leader>ox", M.reject_all,
    { buffer = scratch, desc = "OpenCode diff: reject all" })
