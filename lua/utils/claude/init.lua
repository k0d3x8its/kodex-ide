...
local function ensure_available()
	...
	vim.notify("claude not found at ~/.local/bin/claude — install Claude Code", vim.log.levels.ERROR)
	return false
end
...
function set_bottom_pad(rows)
	local buf = state.panel_buf
	if not (buf and vim.api.nvim_buf_is_valid(buf) and state.pad_ns) then
		return
	end
	...
	if widgets.fit_subagent_view then
		widgets.fit_subagent_view()
	end
	local is_panel = state.panel_win
...
local function reanchor_pad()
	if
		(state.chat_pad or 0) > 0
		or (widgets.todo_height and widgets.todo_height() > 0)
		or (widgets.subagent_height and widgets.subagent_height() > 0)
	then
...
local function render_queue()
	local buf = state.panel_buf
	if not (buf and vim.api.nvim_buf_is_valid(buf) and state.queue_ns) then
		return
	end
...
local function in_typing_phase()
	return state.working and not state.tool_run and not state.perm and not state.diff_pending and not state.compact_mark
end
...
local function gated()
	return state.perm ~= nil
		or state.prewrite ~= nil
		or state.qask ~= nil
		or state.diff_card ~= nil
		or state.diff_pending == true
end
mod._gated = gated
...
local function patch_banner(model, raw_ver)
	...
	local buf = state.panel_buf
	if not (buf and vim.api.nvim_buf_is_valid(buf) and vim.api.nvim_buf_line_count(buf) > BANNER_L1) then
		return
	end
	vim.bo[buf].modifiable = true
	if ver ~= "" then
	...
	if model ~= "" then
		vim.api.nvim_buf_set_lines(buf, BANNER_L1, BANNER_L1 + 1, false, { BANNER_G2 .. BANNER_P2 .. model })
	end
	vim.bo[buf].modifiable = false
	if ver ~= "" then
...
local function update_banner_cwd(cwd)
	...
	if vim.api.nvim_buf_line_count(buf) <= BANNER_L2 then
		return
	end
	vim.bo[buf].modifiable = true
	vim.api.nvim_buf_set_lines(buf, BANNER_L2, BANNER_L2 + 1, false, { BANNER_G3 .. BANNER_P3 .. (cwd or "") })
	vim.bo[buf].modifiable = false
...
local function update_banner_model(friendly)
	...
	if vim.api.nvim_buf_line_count(buf) <= BANNER_L1 then
		return
	end
	vim.bo[buf].modifiable = true
	vim.api.nvim_buf_set_lines(buf, BANNER_L1, BANNER_L1 + 1, false, { BANNER_G2 .. BANNER_P2 .. (friendly or "") })
	vim.bo[buf].modifiable = false
	hl_range(BANNER_L1, 0, #BANNER_G2, "ClaudeHeader")
	hl_range(BANNER_L1, #BANNER_G2 + #BANNER_P2, -1, "ClaudeModel")
end
...
gate.wire({
	...
	-- Cross-type decision-card drain: a queued AskUserQuestion event hands off through
	-- question.show_question_card (see resolve_permission's qask_queue drain).
	show_question_card = question.show_question_card,
})
...
question.wire({
	...
	-- Cross-type decision-card drain: a queued permission event hands off through
	-- gate.show_permission_card (see close_question_card's perm_queue drain).
	show_permission_card = gate.show_permission_card,
	-- Cross-type decision-card drain: a queued diff-review event hands off through
	-- gate.show_diff_card (see close_question_card's diffcard_queue drain).
	show_diff_card = gate.show_diff_card,
})
...
local function set_panel_keymaps(buf)
	...
	vim.keymap.set("n", "<Up>", function()
		nav_or_default(-1, "<Up>")
	end, { buffer = buf, noremap = true, silent = true, desc = "Claude: subagent switcher up" })
	vim.keymap.set("n", "<Down>", function()
		nav_or_default(1, "<Down>")
	end, { buffer = buf, noremap = true, silent = true, desc = "Claude: subagent switcher down" })
...
local function host_file_of(win)
	...
	if ft == "alpha" or ft == "NvimTree" or ft == "claude" then
		return nil
	end
...
local function attach_host_context(text)
	if not state.host_ctx_enabled then
		return text, nil
	end
...
function mod.toggle_host_ctx()
	...
	vim.notify(
		"Claude open-buffer context " .. (state.host_ctx_enabled and "ON" or "OFF") .. " (remembered across restarts)",
		vim.log.levels.INFO
	)
end
...
local function submit(text)
	...
	if command then
		if want_steer and steer(command) then
			return
		end
		...
		if want_steer and steer(text) then
			return
		end
		if state.working then
			enqueue(text)
...
local function open_chat_float(title, callback, opts)
	...
	local border_hl = mode_border_hl()
	vim.wo[win].winhighlight = "FloatBorder:" .. border_hl .. ",FloatTitle:" .. border_hl .. ",NormalFloat:ClaudeBarBg"
	...
	local function close()
		...
		if slash.active() then
			slash.close()
		end -- tear down the "/" menu with the bar
		...
		if state.panel_win and vim.api.nvim_win_is_valid(state.panel_win) then
			...
			vim.schedule(function()
				if
					state.panel_win
					and vim.api.nvim_win_is_valid(state.panel_win)
					and vim.api.nvim_get_current_win() == state.panel_win
				then
	...
	vim.fn.prompt_setcallback(ibuf, function(text)
		submitted = true
		if persist_draft then
			state.chat_draft = ""
		end -- sent → drop the saved draft
...
function mod.prompt_input()
	...
	if state.perm then
		vim.notify("⚠ Permission required — ←/→ select, <CR> confirm, <Esc> reject", vim.log.levels.WARN)
		return
	end
...
function mod.interrupt()
	...
	state.working = false
	stop_spinner()
	if not state.diff_pending then
		clear_hint()
	end
...
local function panel_wheel_down()
	...
	pcall(vim.api.nvim_win_call, win, function()
		...
		if gap then
			...
			if room <= 0 then
				return
			end -- already at the limit → swallow the notch
			if room < step then
				step = room
			end
		end
...
local function open_panel_window(buf)
	...
	vim.api.nvim_create_autocmd("WinEnter", {
		...
		callback = function()
			...
			vim.schedule(function()
				if not vim.api.nvim_win_is_valid(mwin) then
					focuslog("sched:modal-gone", mwin)
					return
				end
				if vim.api.nvim_get_current_win() ~= state.panel_win then
					focuslog("sched:not-panel", mwin)
					return
				end
	...
	if state.panel_buf and vim.api.nvim_buf_is_valid(state.panel_buf) then
		vim.keymap.set("n", "<ScrollWheelDown>", panel_wheel_down, { buffer = state.panel_buf, silent = true })
		...
		vim.keymap.set("n", "<ScrollWheelUp>", panel_wheel_up, { buffer = state.panel_buf, silent = true })
	end
...
function mod.toggle(root_override)
	...
	if ok and opencode.state and opencode.state.opencode_active then
		opencode.toggle()
		vim.notify("Claude panel open — OpenCode closed (<leader>oc to switch)", vim.log.levels.INFO)
	end
	...
	if state.claude_active and state.panel_win and vim.api.nvim_win_is_valid(state.panel_win) then
		stop_process()
	...
	if fresh then
		...
		render_banner(state.model or "", binary_version(), vim.fn.fnamemodify(state.stored_root or root, ":~"))
	end
	clear_hint()
...
function mod.reset()
	...
	state.diff_queue = {}
	-- Cross-type queue for a diff card deferred behind a live perm/qask card
	-- (gate.lua show_diff_card) — holds {path,kind}, not a request_id, so nothing
	-- hangs the CLI if it survives a reset, but a stale entry would spuriously pop
	-- a diff card on the next session's first perm/qask resolve (advisor 2026-07-22).
	state.diffcard_queue = {}
	state.queue = {}
	state.working = false
...
function mod.pick_model()
	vim.ui.select(MODEL_CHOICES, {
		...
		vim.notify(
			"Claude model → " .. choice.label .. (had_session and "  (new session — context reset)" or ""),
			vim.log.levels.INFO
		)
	end)
end
...
local function apply_effort(level)
	...
	stop_process()
	vim.notify(
		"Claude effort → " .. level .. (had_session and "  (new session — context reset)" or ""),
		vim.log.levels.INFO
	)
end
...
function mod.pick_advisor(id)
	if id and ADVISOR_MODELS[id] then
		apply_advisor(id)
		return
	end
	if id == "off" or id == "none" then
		apply_advisor(nil)
		return
	end
...
function mod.current_model()
	if state.model_display and state.model_display ~= "" then
		return state.model_display
	end
	local fm = state.model and friendly_model(state.model) or ""
	if fm ~= "" then
		return fm
...
function mod.toggle_plan()
	...
	stop_process()
	vim.notify(
		"Claude Plan mode " .. (on and "ON" or "OFF") .. (had_session and "  (new session — context reset)" or ""),
		vim.log.levels.INFO
	)
end
...
function mod.on_diff_open(info)
	...
	if path then
		...
		if not ok then
			vim.notify("Claude: diff review card failed to render (" .. tostring(err) .. ")", vim.log.levels.ERROR)
		end
	end
end
...
function mod.on_diff_close()
	...
	close_diff_card()
	-- close_diff_card may have handed off to a queued perm/question card — either
	-- just now (its own return), or on an EARLIER call (resolve_diff_card calls
	-- close_diff_card directly before accept_all/reject_all, which lands back here
	-- via claude_diff's close_diff → on_diff_close a second time). Check live state
	-- rather than trust this call's own return, so the second pass doesn't clobber a
	-- card the first pass already drained into (Gate-3 finding, advisor 2026-07-22)
	-- — that card owns the hint/reopen-bar lifecycle now.
