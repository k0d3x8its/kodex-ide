...
local renders = {}
pet.render = function(new, prev)
	table.insert(renders, { new = new, prev = prev })
end
...
H.check("P-CLS4 Bash rm → cleaning", pet.classify("Bash", { command = "rm -rf build/" }) == "cleaning")
H.check("P-CLS5 Bash mv → cleaning", pet.classify("Bash", { command = "mv old.lua new.lua" }) == "cleaning")
H.check("P-CLS6 Bash pytest → debugging", pet.classify("Bash", { command = "pytest tests/ -x" }) == "debugging")
H.check("P-CLS7 Bash make test → debugging", pet.classify("Bash", { command = "make test" }) == "debugging")
H.check(
	"P-CLS8 debugging beats nothing; test+rm resolves to debugging first",
	pet.classify("Bash", { command = "cargo test && rm target/x" }) == "debugging"
)
H.check("P-CLS9 plain Bash → reading (generic)", pet.classify("Bash", { command = "ls -la" }) == "reading")
...
H.check("P-EVT6 tool_use Read → reading", pet.emit("tool_use", { name = "Read", input = {} }) == "reading")
H.check(
	"P-EVT7 tool_use Bash rm → cleaning",
	pet.emit("tool_use", { name = "Bash", input = { command = "rm x" } }) == "cleaning"
)
H.check(
	"P-EVT8 tool_use Edit → building (write in progress)",
	pet.emit("tool_use", { name = "Edit", input = {} }) == "building"
)
H.check(
	"P-EVT8b tool_use unknown keeps prior state (nil classify)",
...
H.check("P-INT1 interrupt drops work → error sprite", pet.emit("interrupt", { now = 0 }) == "error")
...
H.check("P-INT1d trailing abort result stays error", pet.emit("result", { ok = false }) == "error")
...
H.check("P-INT2 queued turn's thinking event clears error → new activity", pet.emit("thinking") == "thinking")
...
reset()
pet.emit("typing")
H.check("P-SUB1 subagent active surfaces over typing", pet.emit("subagent", { active = true }) == "subagent")
H.check("P-SUB2 subagent clear falls back to typing", pet.emit("subagent", { active = false }) == "typing")
...
-- Schedule: idle 0–15s, groove 15–45s (30s), hero 45–60s (cool-pose flourish), sleep 60s+.
...
H.check("P-IDLE4 t=50 → hero (post-groove)", pet.advance(50) == "hero")
...
H.check("P-RND2 render carries prev", renders[2].prev == "typing" and renders[2].new == "thinking")
