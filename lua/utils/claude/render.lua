...
local function render_user(text, note, steer)
	...
	while idx <= #raw do
		...
		if is_fence(line) then
			...
			if prose_row == 1 then
				hl_range(ln, 0, #USER_ARROW, "ClaudeArrow")
			end
			idx = idx + 1
		end
	end
...
local function count_added_removed(old, new)
	...
	for _, l in ipairs(vim.split(old or "", "\n", { plain = true })) do
		pool[l] = (pool[l] or 0) + 1
		oldn = oldn + 1
	end
	for _, l in ipairs(vim.split(new or "", "\n", { plain = true })) do
		newn = newn + 1
		if (pool[l] or 0) > 0 then
			pool[l] = pool[l] - 1
			common = common + 1
		end
	end
...
local function corner_one_line(s)
	...
	if vim.fn.strdisplaywidth(s) > 68 then
		s = vim.fn.strcharpart(s, 0, 67) .. "…"
	end
	return s
end
...
local function tool_lines(name, input)
	...
	elseif EDIT_NAMES[name] then
		...
		return "● Editing " .. vim.fn.fnamemodify(path or "", ":t") .. file_glyph(path), change_summary(a, r)
	...
	elseif name == "Grep" then
		return "● Searching",
			corner_one_line(input.pattern) .. (input.path and ("  ·  " .. rel_path(input.path)) or "")
...
local function render_tool(name, input)
	...
	if #lines > 1 then
		hl_lines(first + 1, first + 1, "ClaudeDim")
	end
...
local function reset_hunk_fg_cache()
	for k in pairs(hunk_fg_cache) do
		hunk_fg_cache[k] = nil
	end
end
...
local function render_edit_hunk(path, old_lines, new_lines, anchor_row)
	...
	for _, dl in ipairs(vim.split(ud, "\n", { plain = true })) do
		...
		elseif oldno then
			...
			if sign == "+" then
				if changed < HUNK_CAP then
					rows[#rows + 1] = { kind = "add", num = newno, text = dl:sub(2) }
					changed = changed + 1
				else
					dropped = dropped + 1
				end
			...
			elseif sign == "-" then
				if changed < HUNK_CAP then
					rows[#rows + 1] = { kind = "del", num = oldno, text = dl:sub(2) }
					changed = changed + 1
				else
					dropped = dropped + 1
				end
			...
			elseif sign == " " then
				if changed < HUNK_CAP then
					rows[#rows + 1] = { kind = "ctx", num = newno, text = dl:sub(2) }
				end
				oldno = oldno + 1
				newno = newno + 1
			end
	...
	if
		anchor_row
		and anchor_row >= 0
		and anchor_row < buf_lc
		and (vim.api.nvim_buf_get_lines(buf, anchor_row, anchor_row + 1, false)[1] or "") == ""
	then
	...
	for i, d in ipairs(disp) do
		...
		if d.sep then
			...
			if d.dim_gutter then
				hl_range(ln, ind_b, ind_b + gw, d.numhl or "ClaudeDim")
			end
...
local function write_body_lines(entry, limit)
	...
	for i = 1, shown do
		...
		repeat
			...
			local spans = {}
			if firstrow then
				spans[#spans + 1] = { #ind, #ind + gw, "ClaudeDim" }
			end -- gutter number
			if runs then -- clip each token run to this chunk's byte span
				for _, s in ipairs(runs) do
					local cs = math.max(s[1], boff)
	...
	if limit and total > shown then
		...
		lines[#lines + 1] =
			string.format("%s… +%d %s (ctrl+o to expand)", RES_INDENT, hidden, hidden == 1 and "line" or "lines")
...
local function build_collapsed(entry)
	if entry.kind == "write" then
		return write_body_lines(entry, WRITE_HEAD_K)
	end
	...
	if entry.summary then
		...
		for i, r in ipairs(rows) do
			...
			hls[i] = corner
					and {
						{ 0, #RES_CORNER, "ClaudeDim" }, -- dim corner
						{ #RES_CORNER, #RES_CORNER + ck, "ClaudeAdvisor" }, -- green ✔
						{ #RES_CORNER + ck, -1, "ClaudeDim" },
					} -- dim text
	...
	local lines, hls = {}, {}
	for i = 1, n_shown do
		if #lines >= ROW_BUDGET then
			hidden = #entry.body - (i - 1)
			break
		end
		...
		for ri, r in ipairs(wrap_disp(clean, avail)) do
			...
			hls[#lines] = corner and { { 0, #RES_CORNER, "ClaudeDim" }, { #RES_CORNER, -1, group } }
				or { { 0, -1, group } }
		end
	end
	if hidden > 0 then
...
local function build_expanded(entry)
	if entry.kind == "write" then
		return write_body_lines(entry, nil)
	end
	if entry.lang then
	...
	for i, l in ipairs(entry.body) do
		...
		hls[i] = corner and { { 0, #RES_CORNER, "ClaudeDim" }, { #RES_CORNER, -1, group } } or { { 0, -1, group } }
	end
	return lines, hls
end
...
local function result_code_meta(meta, body)
	if not (meta and meta.path) then
		return nil, nil
	end
	if meta.name ~= "Read" and meta.name ~= "NotebookRead" then
		return nil, nil
	end
...
local function render_tool_result(content, is_error, meta, opts)
	...
	state.tool_result_ns = state.tool_result_ns or vim.api.nvim_create_namespace("claude_tool_result")
	...
	entry.end_mark = vim.api.nvim_buf_set_extmark(buf, state.tool_result_ns, first + #lines - 1, 0, {})
...
local function paint_compact_line()
	local buf = state.panel_buf
	if not (buf and vim.api.nvim_buf_is_valid(buf) and state.compact_mark) then
		return
	end
...
local function start_compact_modal()
	if state.compact_mark then
		return
	end -- already compacting (idempotent)
	remove_typing_ph()
	local buf = state.panel_buf
	if not (buf and vim.api.nvim_buf_is_valid(buf)) then
		return
	...
	state.compact_mark = vim.api.nvim_buf_set_extmark(buf, compact_ns(), first, 0, { right_gravity = false })
	compact_spin_i = 1
...
local function finish_compact_modal(meta)
	...
	if not (buf and vim.api.nvim_buf_is_valid(buf) and mark) then
		return
	end
	...
	if type(meta) == "table" then
		...
		local dropped = meta.cumulative_dropped_tokens
			or (type(meta.pre_tokens) == "number" and type(meta.post_tokens) == "number" and (meta.pre_tokens - meta.post_tokens))
			or nil
		line = string.format(
			"✓ Compacted %s → %s tokens%s · %s",
			fmt_ktok(meta.pre_tokens),
			fmt_ktok(meta.post_tokens),
			dropped and (" (−" .. fmt_ktok(dropped) .. ")") or "",
			trig
		)
...
local function render_rate_limit(info)
	...
	if state.rate_limit_shown == key then
		return
	end -- same limit already shown
	...
	local ltype = (type(info.rateLimitType) == "string") and (" (" .. info.rateLimitType:gsub("_", " ") .. ")") or ""
	local first = vim.api.nvim_buf_line_count(buf)
	buf_append({
...
local function render_write_body(path, content)
	...
	if #code > 1 and code[#code] == "" then
		table.remove(code)
	end -- drop trailing newline's empty line
	if #code == 0 then
		return
	end
...
local function render_search(sd, id)
	...
	state.search_ns = state.search_ns or vim.api.nvim_create_namespace("claude_search")
...
local function render_search_result(sb, content, is_error, meta)
	...
	if sd.verb == "Listing" then
		...
		set_header(
			string.format("● Searching for 1 pattern, reading %d %s%s", m, m == 1 and "file" or "files", expand)
		)
	end
...
function Render.expand_result()
	...
	if not (state.tool_results and state.tool_result_ns) then
		return
	end
	...
	e.end_mark = vim.api.nvim_buf_set_extmark(buf, state.tool_result_ns, pick.s + #lines - 1, 0, {})
...
local function render_result(_text)
	local buf = state.panel_buf
	if not (state.turn_t0 and buf and vim.api.nvim_buf_is_valid(buf)) then
		return
	end
	...
	local word = (state.flavor_idx and FLAVOR_DONE[state.flavor_idx]) or FLAVOR_DONE[math.random(#FLAVOR_DONE)]
...
local function subagent_by_id(id)
	if not (id and state.subagents) then
		return nil
	end
...
local function emit_subagent_state()
	if pet_emit then
		pet_emit("subagent", { active = any_subagent_running() })
	end
end
...
local function subagent_by_task(task_id)
	if not (task_id and state.subagents) then
		return nil
	end
	for _, s in ipairs(state.subagents) do
		if s.task_id == task_id then
			return s
		end
	end
	return nil
...
local function render_subagent_inline(event, sub)
	if (event.type or "") ~= "assistant" then
		return
	end
	for _, b in ipairs(content_blocks(event.message)) do
		if (b.type or "") == "tool_use" then
			if (sub.main_lines or 0) >= SUBAGENT_MAIN_CAP then
				subagent_cap_pointer(sub)
				return
			end
			local a = b.input or {}
		...
		for _, b in ipairs(content_blocks(ev.message)) do
			...
			if bt == "text" and type(b.text) == "string" and b.text ~= "" then
				for _, ln in ipairs(vim.split(b.text, "\n", { plain = true })) do
					for _, wl in ipairs(wrap_disp(ln, width)) do
						push("  " .. wl, "ClaudeProse")
					end
				end
			elseif bt == "thinking" and type(b.thinking) == "string" and b.thinking ~= "" then
				...
				for _, ln in ipairs(vim.split(b.thinking, "\n", { plain = true })) do
					for _, wl in ipairs(wrap_disp(ln, width - 2)) do
						push("  " .. wl, "ClaudeThink")
					end
				end
		...
		for _, b in ipairs(content_blocks(ev.message)) do
			if (b.type or "") == "tool_result" then
				...
				for _, raw in ipairs(body) do
					...
					for _, wl in ipairs(wrap_disp(raw, math.max(width - 4, 8))) do
						push("    " .. wl, grp)
					end
					shown = shown + 1
				end
...
local function dispatch(event)
	...
	if parent then
		...
		if sub then
			...
			if (not sub.model) and event.type == "assistant" and type((event.message or {}).model) == "string" then
				sub.model = friendly_model(event.message.model)
				if
					sub.header_lnum
					and state.panel_buf
					and vim.api.nvim_buf_is_valid(state.panel_buf)
					and sub.header_lnum < vim.api.nvim_buf_line_count(state.panel_buf)
				then
	...
	if ev_type == "system" and event.subtype == "init" then
		...
		if type(event.slash_commands) == "table" and #event.slash_commands > 0 and not state.slash_commands then
	...
	elseif ev_type == "system" and event.subtype == "task_started" then
		...
		if sub then
			...
			if type(event.description) == "string" then
				sub.desc = event.description
			end
			if type(event.subagent_type) == "string" then
				sub.kind = event.subagent_type
			end
	...
	elseif ev_type == "system" and event.subtype == "task_notification" then
		...
		if sub then
			...
			if type(event.summary) == "string" then
	...
	elseif ev_type == "stream_event" then
		...
		elseif st == "content_block_stop" and state.think_start and se.index == state.think_idx then
		...
		for _, block in ipairs(content_blocks(event.message)) do
			if (block.type or "") == "tool_result" then
				...
				if meta and EDIT_NAMES[meta.name] and meta.path then
					require("utils.claude_diff").resolve_prewrite_result(meta.path, block.is_error == true)
				end
				...
				if meta and (meta.name == "TodoWrite" or meta.name == "TaskCreate" or meta.name == "TaskUpdate") then
	...
	elseif ev_type == "assistant" then
		...
		for _, block in ipairs(content) do
			...
			elseif btype == "tool_use" then
				...
				if pet_emit then
					pet_emit("tool_use", { name = name, input = input })
				end
				...
				elseif sd then
					...
					if
						name == "Write"
						and type(input.content) == "string"
						and input.content ~= ""
						and vim.fn.filereadable(input.file_path or input.path or "") == 0
					then
	...
	elseif ev_type == "result" then
		...
		if pet_emit then
			pet_emit("result", { ok = event.is_error ~= true })
		end
	...
	if render_queue and state.queue and #state.queue > 0 then
		render_queue()
	end
end
