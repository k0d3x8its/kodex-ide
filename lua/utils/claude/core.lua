...
Core.state = {
	...
	chat_buf = nil,
	chat_close = nil,
	-- True while a decision card (permission OR question) is up that dismissed an
	-- open chat bar, so whichever card's resolve/close path runs LAST in a chain
	-- (same-type or cross-type — see the concurrent-permission-modal-ghost cross-type
	-- fix) knows to reopen the bar (draft restored) afterwards. Shared, not per-type:
	-- two separate flags (perm_reopen_bar/qask_reopen_bar) meant a cross-type drain
	-- (perm resolves → drains a queued question, or vice versa) consumed the WRONG
	-- flag — the source type's flag was left stranded true and the destination
	-- type's flag was never set, so the bar silently never reopened.
	decision_reopen_bar = false,
...
function Core.turn_elapsed_ms()
	if not state.turn_t0 then
		return 0
	end
	local paused = state.turn_paused_ms or 0
	if state.pause_t0 then
		paused = paused + (vim.loop.now() - state.pause_t0)
	end
...
function Core.pause_turn()
	if not state.pause_t0 then
		state.pause_t0 = vim.loop.now()
	end
end
...
local function buf_append(lines)
	...
	then
		...
		if (state.pad_rows or 0) > 0 then
			...
			end -- re-place; total recomputed w/ widget
		else
