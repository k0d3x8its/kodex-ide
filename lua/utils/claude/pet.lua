...
local IDLE_STAGES = {
	...
	{ at = 45, phase = "hero" }, -- cool-pose flourish before sleep (45→60): one-shot
	-- 4.4s animation (pet_render's LOOP_FROM holds the last frame), so most of this
	-- window is a held freeze-frame, not looping motion.
...
M.now = function()
	if vim and vim.loop and vim.loop.now then
...
local function active(name, c)
	...
	if name == "building" then
		return c.permission == "build" or c.work == "building"
	end
	if name == "notification" then
		return c.permission == "notify"
	end
	if name == "diff_wait" then
		return c.diff == "wait"
	end
	if name == "diff_rejected" then
		return c.flash == "diff_rejected"
	end
	if name == "diff_approved" then
		return c.flash == "diff_approved"
	end
	if name == "debugging" then
		return c.work == "debugging"
	end
	if name == "cleaning" then
		return c.work == "cleaning"
	end
	if name == "reading" then
		return c.work == "reading"
	end
	if name == "subagent" then
		return c.subagent
	end
	if name == "advising" then
		return c.work == "advising"
	end
	if name == "thinking" then
		return c.work == "thinking"
	end
	if name == "typing" then
		return c.work == "typing"
	end
	if name == "happy" then
		return c.flash == "happy"
	end
	if name == "headphones_groove" then
		return c.idle_phase == "headphones_groove"
	end
	if name == "hero" then
		return c.idle_phase == "hero"
	end
	if name == "idle" then
		return c.idle_phase == "idle"
	end
	if name == "sleep" then
		return true
	end -- floor: always active
	return false
end
...
function M.emit(event, data)
	...
	elseif event == "tool_use" then
		...
		if st then
			c.work = st
		end -- nil (Edit/Write/…) keeps the current state
