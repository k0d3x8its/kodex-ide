...
local function run_shell_in_toggleterm_at_root(buffer_number, shell_command)
	...
	do
		...
		local ok = pcall(
			vim.cmd,
			("TermExec direction=float dir=%s name=%s cmd='%s'"):format(
...
local function show_platformio_actions_menu()
	if not is_platformio_buffer(0) then
		vim.notify(
			"Not a PlatformIO project (no platformio.ini / .pio found).",
			vim.log.levels.WARN,
			{ title = "PlatformIO" }
		)
		return
	...
	callback = function(event)
		...
		if is_platformio_buffer(buffer_number) then
			map.set(
				"n",
				"<leader>p",
				show_platformio_actions_menu,
				keyopts("PlatformIO: Menu", { buffer = buffer_number })
			)
		else
			pcall(vim.keymap.del, "n", "<leader>p", { buffer = buffer_number })
		end
